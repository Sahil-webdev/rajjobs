...
export default function TestEditorPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE}/api/admin/exam-details`, {
        method: "POST",
        headers: {
      ...
      if (result.data?._id) {
        ...
        const fetchResponse = await fetch(
          `${API_BASE}/api/admin/exam-details/${result.data._id}`,
          {
  ...
  return (
    <div style={{ padding: 40, maxWidth: 1200, margin: "0 auto" }}>
      ...
      {/* Instructions */}
      <div style={{ background: "#fef3c7", border: "3px solid #f59e0b", borderRadius: 12, padding: 24, marginBottom: 32 }}>
        <h2 style={{ fontSize: 20, fontWeight: "bold", marginBottom: 16, color: "#92400e" }}>Instructions:</h2>
      ...
      <div style={{ marginBottom: 24 }}>
        <h3 style={{ fontSize: 18, fontWeight: "bold", marginBottom: 16, color: "#1f2937" }}>
          Rich Text Editor:
        </h3>
        <RichEditor
          editorData={editorValue}
          setEditorData={(val: string) => {
            console.log("📝 Editor onChange triggered, length:", val?.length);
            setEditorValue(val);
            addLog(`✏️ Editor updated: ${val?.length || 0} chars`);
          }}
          handleOnUpdate={(val: string) => setEditorValue(val)}
        />
      </div>

      {/* Current State */}
      <div style={{ background: "#eff6ff", borderRadius: 8, border: "1px solid #3b82f6", padding: 16, marginBottom: 24 }}>
        <p style={{ fontSize: 14, fontWeight: "bold", color: "#1e40af", marginBottom: 8 }}>Current Editor State:</p>
        <p style={{ fontSize: 13, fontFamily: "monospace", color: "#1e40af" }}>
          ...
          {editorValue && (
            <>
              ...
              <br />
              HTML: <code style={{ background: "#dbeafe", padding: "2px 6px", borderRadius: 4 }}>{editorValue.substring(0, 100)}</code>
            </>
          )}
        </p>
      </div>
      ...
      {/* Logs */}
      <div style={{ background: "#1f2937", color: "#f9fafb", padding: 24, borderRadius: 12, fontFamily: "monospace", fontSize: 13, lineHeight: 1.8, maxHeight: 600, overflowY: "auto" }}>
        <h3 style={{ fontSize: 16, fontWeight: "bold", marginBottom: 16, color: "#10b981" }}>📋 Debug Logs:</h3>
        {logs.length === 0 ? (
          ...
          logs.map((log, idx) => (
            <div
              ...
              style={{
                marginBottom: 4,
                color: log.includes("❌") ? "#ef4444" : log.includes("✅") ? "#10b981" : log.includes("⚠️") ? "#f59e0b" : log.includes("🚀") ? "#3b82f6" : "#f9fafb",
              }}
            >
              {log}
            </div>
      ...
      {/* Save Result */}
      {saveResult && (
        <div style={{ background: saveResult.success ? "#f0fdf4" : "#fee2e2", border: `2px solid ${saveResult.success ? "#10b981" : "#ef4444"}`, borderRadius: 12, padding: 24, marginTop: 24 }}>
          <h3 style={{ fontSize: 18, fontWeight: "bold", marginBottom: 16, color: saveResult.success ? "#166534" : "#991b1b" }}>
            {saveResult.success ? "✅ Save Response:" : "❌ Error Response:"}
          </h3>
          <pre style={{ background: "white", padding: 16, borderRadius: 8, overflow: "auto", fontSize: 12 }}>
